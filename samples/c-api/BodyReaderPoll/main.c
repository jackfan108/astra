...
void output_bodies(astra_bodyframe_t bodyFrame)
{
    ...
    for(i = 0; i < bodyList.count; ++i)
    {
            ...
            bodyStatus == ASTRA_BODY_STATUS_TRACKING)
        {
            ...
            const astra_body_features_t features = body->features;
            const bool jointTrackingEnabled       = (features & ASTRA_BODY_FEATURES_JOINTS   ) != ASTRA_BODY_FEATURES_NONE;
            const bool handPoseRecognitionEnabled = (features & ASTRA_BODY_FEATURES_HANDPOSES) != ASTRA_BODY_FEATURES_NONE;
